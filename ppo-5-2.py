...
class PPO:
    ...
    def step(self, cap_idx, action_idx,decap_val,):  # 这里action应该是0-4，代表五个方向,decap_val是可选电容值
        str1 = ''
        action_v =  copy(action_space[action_idx]) 
        cap_idxv = copy(cap_idx)
        ...
        cap_idx = copy(cap_idxv)
        for i in range(len(cap_idx)):
            str1 += 'c_decap_%d_%d nd_1_0_%d_%d 0 %e\n' % (i,cap_idx[i],node_loc[cap_idx[i]][0],node_loc[cap_idx[i]][1],decap_val)
        f = open('vdd_decap.1','w')
        f.write(str1)
        f.close()
        run_os()
        state_ = readvdi('chiplet1_vdd_1_vdi.csv')
        self.st = np.append(self.st,np.sum(state_))
...
for eps in range(episodes):
    if eps == 0 :
        state,cap_idx = chip.reset(1e-9)
        ccc = deepcopy(cap_idx)
    ...
    if chip.mode ==0:
        explr += 1
    state_,a,cap_idx1 = chip.step(cap_idx,a,1e-9)
    s_ = norm(state_)
    r = (1.e-10 - np.sum(state_)) / 1.e-10
    chip.store_transition(s,a,r,s_)
    if chip.mem_cnt >0.8* memorysize:
        chip.learn()
    reward.append(r)
    total_vdis.append(np.sum(state_))
    state = state_
    cap_idx_list.append(cap_idx1)
    act.append(a)
    epsilon = 0.01 + 0.99*np.exp(-4. * eps/episodes)
    #if eps < 0.9 *  memorysize:
       # epsilon = 0.9
   # else:
        #epsilon = 0.9* np.exp( -0.03 * (eps - 0.9*memorysize) )
    cap_idx = deepcopy(cap_idx1)
...
np.savetxt('cap_move.txt',cap_array)
np.savetxt('state.sum.txt',chip.st)
cap_move = cap_array[-200:-100,:]
print('q_target',chip.learn_last)
print(exp)
plt.plot([ex for ex in range(len(exp))],exp,label='exploration')
#plt.plot([ex1 for ex1 in range(len(exp))],exp,label='exploration')
plt.title('exploration/exploitation')
plt.ylabel('per 50 eps')
plt.legend()
plt.figure()
#print(cap_move)
#o = input('cap change')
...
plt.plot([0,episodes - 1],[1.e-10,1.e-10],label='target VDI',linestyle='--')
plt.title('VDI plot')
plt.legend()
plt.figure()
plt.subplot(1,2,1)
...
plt.legend()
plt.figure()
plt.plot([s for s in range(len(epsilons))],epsilons)
plt.title('epsilon')
plt.xlabel('episodes')
plt.show()


torch.save(chip.target_net.state_dict(),'target_net_model.pkl')      # save agent parameters
...
plt.plot([b for b in range(len(action1))],action1)
